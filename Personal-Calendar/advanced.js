(() => {
  ...
  const DATA='calm-calendar:v1';
  const META='calm-calendar:advanced:v2';
  ...
  const dateObj=s=>{const [y,m,d]=String(s).split('-').map(Number);return new Date(y,m-1,d)};
  const mins=t=>{if(!t)return 0;const [h,m]=t.split(':').map(Number);return h*60+m};
  const time=m=>`${pad(Math.floor(m/60)%24)}:${pad(m%60)}`;
  const toast=msg=>{const el=document.querySelector('#toast');if(!el)return;el.textContent=msg;el.classList.add('show');clearTimeout(el._advancedTimer);el._advancedTimer=setTimeout(()=>el.classList.remove('show'),2600)};
  const esc=s=>String(s??'').replace(/[&<>\"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));
  const currentEvent=id=>state().events.find(e=>e.id===id);
  const eventIdFromNode=node=>String(node?.dataset?.event||'').split('@')[0];
  const remember=()=>{undoStack.push({state:state(),meta:load(META,{events:{},calendarNotes:{}})});if(undoStack.length>20)undoStack.shift()};
  const setMeta=(id,patch)=>{meta.events[id]={...(meta.events[id]||{}),...patch};save(META,meta)};
  const eventMeta=id=>meta.events[id]||{status:'confirmed',availability:'busy',privacy:'default',category:'General',tags:[],url:'',timeZone:prefs.defaultTimeZone,travelMinutes:prefs.defaultTravel,focus:false};
  ...
  function restoreSnapshot(id){const snap=load(SNAP,[]).find(x=>x.id===id);if(!snap)return;save(DATA,snap.state);save(META,snap.meta);location.reload()}
  function exportMeta(){downloadBlob(new Blob([JSON.stringify({version:2,exportedAt:new Date().toISOString(),meta,prefs},null,2)],{type:'application/json'}),'calm-advanced-metadata.json')}
