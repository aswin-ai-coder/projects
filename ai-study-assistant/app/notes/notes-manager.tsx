...
type Note = { id: string; title: string; content: string; subject: string; updatedAt: string };
...
export default function NotesManager() {
  ...
  function save(event: FormEvent) {
    ...
    setNotes((current) => editing?.id ? current.map((note) => note.id === editing.id ? next : note) : [next, ...current]);
    setEditing(null);
  }
  function remove(id: string) { setNotes((current) => current.filter((note) => note.id !== id)); if (editing?.id === id) setEditing(null); }

  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1"><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search notes..." className="w-full rounded-xl border border-slate-800 bg-slate-900 px-4 py-3 text-sm outline-none focus:border-slate-500" /></div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        {filtered.map((note) => (
          <article key={note.id} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
            <div className="flex items-start justify-between gap-4"><div><span className="text-xs text-slate-500">{note.subject}</span><h2 className="mt-1 text-lg font-bold">{note.title}</h2></div><span className="text-xs text-slate-600">{note.updatedAt}</span></div>
            <p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-400">{note.content}</p>
            <div className="mt-5 flex gap-2"><button onClick={() => startEdit(note)} className="rounded-lg border border-slate-700 px-3 py-2 text-xs font-semibold hover:border-slate-500">Edit</button><button onClick={() => remove(note.id)} className="rounded-lg border border-slate-800 px-3 py-2 text-xs font-semibold text-slate-500 hover:border-slate-600 hover:text-slate-300">Delete</button></div>
          </article>
        ))}
      </div>
