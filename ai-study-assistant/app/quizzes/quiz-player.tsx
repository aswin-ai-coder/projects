...
export default function QuizPlayer() {
  const [current, setCurrent] = useState(0); const [selected, setSelected] = useState<number | null>(null); const [answers, setAnswers] = useState<(number | null)[]>(Array(questions.length).fill(null)); const [finished, setFinished] = useState(false);
  ...
  function next() { if (current === questions.length - 1) { const attempt: QuizAttempt = { id: Date.now().toString(), title: "Quick Quiz", score, total: questions.length, date: new Date().toLocaleString() }; const history = readStorage<QuizAttempt[]>(QUIZ_HISTORY_KEY, []); writeStorage(QUIZ_HISTORY_KEY, [attempt, ...history]); setFinished(true); } else { setCurrent(current + 1); setSelected(answers[current + 1]); } }
  ...
  return <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 sm:p-8"><div className="flex items-center justify-between text-sm text-slate-500"><span>Question {current + 1} of {questions.length}</span><span>{Math.round((current / questions.length) * 100)}% complete</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-800"><div className="h-full rounded-full bg-white transition-all" style={{ width: `${((current + 1) / questions.length) * 100}%` }} /></div><h2 className="mt-8 text-2xl font-bold leading-9">{question.question}</h2><div className="mt-6 grid gap-3">{question.options.map((option, index) => <button key={option} onClick={() => choose(index)} className={`rounded-xl border px-4 py-4 text-left text-sm transition ${selected === index ? "border-white bg-white text-slate-950" : "border-slate-700 hover:border-slate-500"}`}><span className="mr-3 font-semibold">{String.fromCharCode(65 + index)}.</span>{option}</button>)}</div><div className="mt-7 flex justify-end"><button disabled={selected === null} onClick={next} className="rounded-xl bg-white px-6 py-3 text-sm font-semibold text-slate-950 disabled:cursor-not-allowed disabled:opacity-30">{current === questions.length - 1 ? "Finish quiz" : "Next question"}</button></div></section>;
}
